<template>
  <BaseCard data-testid="widget-ossd-status">
    <BaseSectionTitle title="Lichtgitter OSSD" subtitle="Aktueller Status & letzte Interrupts" />

    <div class="osss-panel">
      <OSSDIndicatorRow title="Lichtgitter 1" :items="row1" />
      <OSSDIndicatorRow title="Lichtgitter 2" :items="row2" />
    </div>

    <h3 class="osss-panel__subheadline">Letzte 4 Interrupts</h3>
    <InterruptHistoryTable data-testid="widget-ossd-history" :interrupts="lastInterrupts" />
...
const sortedInterruptsDesc = computed(() =>
  [...props.interrupts].sort((a, b) => new Date(b.time).getTime() - new Date(a.time).getTime()),
)
...
function isActive(lichtgitterNr: LichtgitterNr, ossdNr: OssdNr): boolean {
  const latest = sortedInterruptsDesc.value.find(
    (i) => i.lichtgitterNr === lichtgitterNr && i.ossdNr === ossdNr,
  )
  if (!latest) {
    return false
  }
  return latest.ossdStatus === 'E'
}
